...
const AllFollowings = () => {
  ...
  // const UserSearch = followings.map((item)=>{
  //   return item.following_user.first_name
  // })
  ...
  return (
    <Grid container rowGap={1} justifyContent="center">
      <Grid item xs={12} p={1} textAlign="center">
        <p style={{ fontSize: "30px" }}>Followings</p>
      </Grid>
      <Grid item xs={12} p={1} textAlign="center">
        <Box
          sx={{
      ...
      {/* create a search bar for search following */}
      {followings
        ?.filter((props) => {
          return (
            props.following_user.first_name
              ?.toLowerCase()
              ?.includes(search?.toLowerCase()) ||
            props.following_user.last_name
              ?.toLowerCase()
              ?.includes(search?.toLowerCase())
          );
        })
        .map((props) => (
          <Grid item xs={6} sm={3} md={2} p={1} key={props.following_user.id}>
